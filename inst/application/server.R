server <- function(input, output, session){
  ...
  # SEIR Model Tab ----------------------------------------------------------
  observe({
    scenario <- input$scenarios

    R0_scenario <- if (scenario == "a" | scenario == "e") {
      1.34
    } else if (scenario == "b") {
      5.36
    ...
    eir_scenario <- if (scenario == "a" | scenario == "d") {
      .001
    } else if (scenario == "b") {
      0.05
    } else if (scenario == "c" | scenario == "e") {
      0.01
    }
  ...
  SEIR_inputs <- reactive({
    SEIR_to_plot(selected_fire = input$SEIR_fire,
                 Rnot_ = input$R0,
                 eir_ = input$eir)
  })

  output$SEIR_bar_all <- renderPlot({
    SEIR_plot(fire_params = SEIR_inputs(),
              plot_type = "all_states",
              log_scale = FALSE,
              legend_pos = c(.8, .9),
              title = "Susceptable, Exposed, Infectious, and Removed Personnel")
  })

  output$SEIR_bar_inf <- renderPlot({
    SEIR_plot(fire_params = SEIR_inputs(),
              plot_type = "infected_states",
              log_scale = FALSE,
              legend_pos = c(.8, .9),
              title = "Exposed, Infectious, and Removed Personnel")
  })

  output$SEIR_line_nonlog <- renderPlot({
    cumulative_inf(fire_params = SEIR_inputs(),
                   log_scale = FALSE,
                   legend_pos = c(.8, .9),
                   title = "Total Personnel and Cumulative Infected Personnel")
  })

  output$SEIR_line_log <- renderPlot({
    cumulative_inf(fire_params = SEIR_inputs(),
                   log_scale = TRUE,
                   legend_pos = c(.8, .9),
                   title = "Total Personnel and Cumulative Infected Personnel (Log y scale)")
  })
}
